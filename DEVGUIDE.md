# Shutdown-Cleanup Module Developer Guide
...
By default, the module listens to common signals (`SIGINT`, `SIGTERM`, `SIGHUP`, and `exit`) to initiate the shutdown process. Customize this behavior as needed:
...
Gain flexibility by registering handlers for specific signals, allowing for custom logic without necessitating a shutdown:
...
registerSignalHandler(
  ...
  false,
) // <= false indicates that the application should not terminate
```
...
- **Error Handling Strategies:** Choose between continuing with remaining handlers after an error (`'continue'`) or stopping the shutdown process (`'stop'`).
...
registerHandler(
  ...
  'closeConnections',
  2, // <= Specify the phase number, 1 is the default phase and the highest priority
)

// Add extra signal to listen for
addSignal('beforeExit')
...
Adds a new signal to the list that, when received, will initiate the shutdown process.
...
Registers a shutdown handler that will be executed when the shutdown process begins. Handlers can be organized into phases. If no name is provided, the handler will be registered with a unique identifier. If no phase is specified, the handler will be executed in the default phase.
...
Registers a handler for a specific signal. This allows for custom logic to be executed without necessarily terminating the application.
...
Provides a list of all signals that are currently being listened to by the module.
